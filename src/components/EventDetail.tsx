...
import type { BaseballFixture, Fixture, FootballFixture } from '@/data/types';
import { formatTime } from '@/lib/scores';
import { Icon } from './Icon';
import { Status, SportScore } from './ScoreCard';

function PeriodTable({ event }: { event: BaseballFixture | FootballFixture }) {
...
function SportDetail({ event }: { event: Fixture }) {
  switch (event.sport) {
    ...
    case 'tennis': return <section className="detail-section"><h2>{event.status === 'live' ? 'On court' : 'Match information'}</h2><dl className="detail-facts"><div><dt>Event</dt><dd>{event.category}’s singles</dd></div><div><dt>Round</dt><dd>{event.round}</dd></div><div><dt>Format</dt><dd>Best of {event.category === 'Men' ? '5' : '3'} sets</dd></div>{event.duration && <div><dt>Time on court</dt><dd>{event.duration}</dd></div>}{event.status === 'live' && <div><dt>Serving</dt><dd>{event.participants[event.server ?? 0].name}</dd></div>}<div><dt>Surface</dt><dd>Hard court · Outdoor</dd></div></dl></section>;
    case 'baseball': return <><PeriodTable event={event}/><section className="detail-section"><h2>{event.status === 'scheduled' ? 'Probable pitchers' : 'Starting pitchers'}</h2><dl className="detail-facts">{event.pitchers.map((pitcher, i) => <div key={pitcher}><dt>{event.participants[i].name}</dt><dd>{pitcher}</dd></div>)}</dl></section></>;
    case 'football': return <><PeriodTable event={event}/>{event.status === 'live' && <section className="detail-section"><h2>Current possession</h2><div className="possession-panel"><span className="possession-dot"/><strong>{event.participants[event.possession ?? 0].name}</strong><span>{event.situation}</span></div></section>}</>;
  }
}
