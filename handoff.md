...
32. [ ] Authenticated Live Tennis API Free feasibility spike — Codex, GPT-5.6 Sol / high reasoning
    ...
    - The authenticated surveys confirm ATP/WTA live and upcoming singles, US Open ATP/WTA identity, and a known live match remaining readable through final on Free. Four of ten distinct ATP live changes were observed, and one WTA live score observation is now captured; tiebreak and exceptional statuses remain outstanding, so the gate stays open.
...
38. [x] Free-tier quota and freshness coordinator — Claude, GPT-5.6 Sonnet / high reasoning
...
39. [x] Opt-in real tennis scoreboard vertical slice — Claude, GPT-5.6 Sonnet / high reasoning
...
Task #38 added `tests/refresh-repository.test.ts`, a synthetic Drizzle transaction harness for `createDrizzleRefreshRepository` in the same hand-rolled fake-`db` style as `tests/normalization-repository.test.ts` — operation-sequence logging, pre-configured row state, no real WHERE-predicate evaluation, no PostgreSQL service required. It proves the four behaviors this task named: quota reservation (a granted lease increments `provider_daily_budgets.calls` by exactly `reserveCalls`; a request that would exceed `LIVE_TENNIS_DAILY_CALL_LIMIT` is refused without mutating either row); one concurrent lease owner (an active lease blocks a second `acquire`, an expired one does not and yields a fresh token); token-checked release (`complete`/`fail` report success only when the `RETURNING` result reflects a token match, and `false` for a stale/mismatched token); and accepted/failure state transitions (exact `UPDATE` payload assertions for both paths, including `nextRefreshAt` advancing through the resource's cadence on success versus honoring the caller's `retryAt` on failure). It also adds a regression test for the UTC-day fix in `bb810a4` (`now.slice(0, 10)` vs `utcDay(now)`), using a non-UTC-offset instant where the local and UTC calendar dates differ — verified against the actual bug by temporarily reverting the fix, confirming the new test failed, then restoring it and confirming the suite passed again, rather than trusting the assertion looked right. `npm run typecheck`, `npm run lint`, `npm test` (75/75), `npm run db:check`, and `npm run build` all pass. No route wiring, no Live Tennis calls from product runtime, and no Redis/workers/cron/SSE/WebSockets/polling were added.

Between #38 and #39, the PostgreSQL blocker was resolved: the user provisioned a dedicated Supabase project (`slate`) and supplied its transaction-pooler connection string via `DATABASE_URL` in `.env.local` (gitignored, alongside `LIVE_TENNIS_API_KEY`; never committed or printed). `src/db/client.ts` added `getDb()` — a `postgres` (postgres.js) client wrapped in `drizzle-orm/postgres-js`, with `prepare: false` since Supabase's transaction pooler does not support server-side prepared statements — and a `db:migrate` script applied the existing migrations to the live instance, confirmed via an independent `information_schema` query rather than trusting the migrate command's own success message. `client.ts` intentionally has no `server-only` guard: `src/db` stays the neutral, directly-testable layer it already was, and the server-only boundary belongs at the composition root. `tests/architecture.test.ts`'s allowlist was extended for `db`'s now-legitimate `postgres`/`drizzle-orm/postgres-js` imports — the first real driver instantiation in that layer.

Task #39 relocated `RefreshRepository`'s contract to `src/application/refresh-repository.ts` (same reasoning as #38's note above: the new orchestration logic needed to depend on it from `application/`, which cannot import `db/`), added `src/application/live-tennis-refresh.ts` (the request-driven refresh cycle, injectable and synthetically tested — see `tests/live-tennis-refresh.test.ts`) and `src/application/merge-tennis-graph.ts` (replaces mock tennis with real tennis while leaving every other sport untouched — see `tests/merge-tennis-graph.test.ts`, including a cold-start case), then wired both together in `src/server/live-tennis.ts` and `src/server/scoreboard.ts` behind the environment-based opt-in described in "Current Phase 2 boundary" above. Full detail — including the three real bugs this surfaced (unseeded `sports` table, static prerendering that would have frozen one real-data snapshot into the served HTML forever, and a pooled-connection statement timeout in the pre-existing whole-graph read) and how each was confirmed and fixed — is in the `feat: wire real tennis data into the running scoreboard (#39)` commit message. Verified end-to-end against the real Supabase project and Live Tennis API: a live WTA US Open quarterfinal and four other real matches render correctly, merged with untouched mock data, and removing the env vars restores byte-for-byte original mock behavior. `npm run typecheck`, `npm run lint`, `npm test` (84/84), `npm run db:check`, and `npm run build` all pass.
...
Phase 1 is complete. Real tennis data now reaches the UI: `getInitialScoreboardData` in `src/server/scoreboard.ts` branches on `isLiveTennisModeEnabled()` (both `DATABASE_URL` and `LIVE_TENNIS_API_KEY` set — an environment-level opt-in; no UI toggle exists or is planned). With neither var set, mock startup is byte-for-byte unchanged — confirmed, not assumed. With both set, a request-driven refresh cycle (`src/application/live-tennis-refresh.ts`, driven from `src/server/live-tennis.ts`) tries the three resources named in `docs/phase-2.md`'s free-slice scope (`live_matches`, `upcoming_matches`, `fixtures`), and `src/application/merge-tennis-graph.ts` replaces mock tennis with whatever the real database currently has, leaving soccer/MLB/NFL untouched.

### Immediate Claude starting point

#39 is complete and verified end-to-end against the real, provisioned Supabase project and Live Tennis API — not just synthetically. A live WTA US Open quarterfinal (Rybakina vs. Zheng) and four other real matches currently render correctly in For You and ATP/WTA, correctly merged alongside untouched mock Tottenham/Red Sox/NFL data.

Three real bugs surfaced only by actually running this, recorded in detail in the `feat: wire real tennis data into the running scoreboard (#39)` commit message: the `sports` reference table was never seeded (every canonical table with a `sport_id` column has a foreign key into it, so every tennis write failed until this was fixed); `app/page.tsx` had no dynamic API usage, so Next.js statically prerendered it at build time — a production build was caught actually spending 5s hitting the real DB/API and would have frozen that one snapshot into the served HTML forever, fixed with `export const dynamic = 'force-dynamic'`; and the pre-existing whole-graph `createDrizzleScoreboardRepository` read occasionally hit a statement timeout under back-to-back testing, bounded at the time by an 8s timeout in `getInitialScoreboardData` that falls back to mock rather than hanging.

That third one's actual root cause was found afterward, once real-mode `npm run dev` started consistently losing the 8s race on every request rather than occasionally. It was not Supabase pooler latency: `readGraph`'s `Promise.all` opens 10 simultaneous connections, and postgres.js's client-side pool defaults to `max: 10` — exactly zero spare capacity, so any request overlapping with another (confirmed here: Next dev auto-reloading an already-open browser tab on server restart, landing a second full request at the same moment as the first) leaves one request's queries queued behind a pool that never frees up, appearing as an indefinite hang rather than added latency - reproduced consistently even against a 25s timeout, and confirmed absent under `next build && next start`, which pointed away from the app logic and toward connection accounting. `src/db/client.ts` now sets `max: 20`; every request measured after that fix completed in 4-5.5s, repeatedly, including deliberately concurrent ones. The 8s timeout in `getInitialScoreboardData` is unchanged and now has real margin instead of none.

The next task is **#40 — Free-slice validation and paid-capability decision**: measure real data quality and product value against the free tier's actual limits before considering a paid provider tier. This is explicitly a product/cost decision, not an implementation task — decide with the user before writing code. The measurement half is done; findings:

- **Live data quality is genuinely good.** A direct call against the real provider (bypassing the app, to avoid double-reporting quota) on 2026-09-09 returned a live WTA US Open quarterfinal (Rybakina vs. Zheng, correct names, set/game/point score, `sources_count: 3`, sub-30-second staleness metadata) and four correctly-identified upcoming ATP/WTA quarterfinals and semifinals (Khachanov–Blockx, Zverev–Van de Zandschulp, Tiafoe–Shelton, Gauff–Andreeva, Pegula–Sabalenka). This is real, accurate, well-structured data at zero cost.
- **Fixed: finished matches were getting stuck showing a stale "live" score forever.** `LiveTennisHttpClient.listMatches`'s type only accepts `status: 'live' | 'upcoming'`; once a real match ends, the provider stops returning it under either filter, our refresh cycle stopped fetching it, and `readGraph` had no staleness/expiry logic — so the canonical row stayed frozen at whatever status it last held, almost always `'live'`, indefinitely. Confirmed live, not hypothetically: the real Rybakina-vs-Zheng US Open quarterfinal used throughout earlier #39 testing had actually finished (Rybakina won) over 80 minutes before its canonical row was ever corrected. A new `match_resolution` refresh resource (30 min cadence, `reserveCalls: 1` to keep an empty cycle's worst-case cost negligible) now finds tennis events still marked `'live'` an hour past their last observation and re-checks each one directly via the provider's `getMatch(matchId)` - already exposed on the client, no new provider access needed - so this needed no product decision, just a new `StaleLiveEventRepository.findStaleLiveEvents` query (`src/db/normalization-repository.ts`) plus the new resource in `refreshResources`/`providerRefreshResourceEnum` (migration `0002_omniscient_the_hand.sql`, additive, applied and verified against the live enum). Fixing it surfaced one more real gap: `getMatch` on that finished match returned `event_status: "Finished"`, a value never in the reviewed `liveTennisEventStatuses` allowlist because no prior free-tier survey had ever queried an actually-completed match - added now, backed by this firsthand observation, per the project's existing decoder philosophy. Re-ran against the real database afterward: the match now reads `status: "final"` with a complete 3-set score and no leftover point/server fields, exactly as `statusFor`/`scoreState` already intended once the value decodes at all.
...
#### Additional-sport provider research (#48)

While answering a direct question about tennis freshness, a live example of the exact rate-limit risk surfaced mid-session: `live_matches` hit the provider's own rate limit and wasn't due to retry for roughly another hour, on top of the already-known 30-minute cadence and shared 80-calls/day budget. That prompted the user to ask whether soccer, MLB, or NFL's free tiers avoid this failure mode before committing further to tennis. Findings, from live web search (not guessed), each sport's strongest free/cheap candidates:

- **Soccer**: **football-data.org** — free, no card, rate-limited at 10 requests/minute rather than a daily cap (structurally more forgiving than tennis's shape), 12 competitions, fixtures, tables; scores reported as "delayed" on the free tier, exact delay not yet verified. **API-Football** — free, no card, but a 100-requests/day cap, the same shape that burned us on tennis; broader league coverage (1,200+) and full endpoint access even for free.
- **MLB**: **MLB Stats API** (`statsapi.mlb.com`) — the league's own public API, free, no key, no registration, for non-commercial/non-bulk use; deep coverage (schedule, box scores, play-by-play, standings, rosters) with no published daily cap for normal use. This is the strongest candidate found across all three sports — it structurally avoids the whole class of problem tennis just hit. SportsData.io is the paid alternative: free trial is eval-only, real access starts at $99–149/month (next-day-delayed data, not live) or $500–1,000+/month for live production — not a first candidate given cost.
- **NFL**: no strong free candidate. The **ESPN hidden API** has a confirmed working live-scoreboard endpoint, free, no key — but it's explicitly unofficial and undocumented, with no published rate limit and no guarantee it won't change or disappear; a different kind of risk than tennis's (stability, not quota). **TheSportsDB** is free and documented but its live in-game score depth for NFL wasn't confirmed. SportsData.io's paid tiers apply here too, same cost concern as MLB.

No decision has been made on which sport (if any) to build next — this is research only, mirroring how `docs/phase-2.md` originally compared tennis providers before any tennis code was written. If MLB is chosen, note that "free and unlimited" still warrants verifying the exact non-commercial/non-bulk terms before relying on it for anything beyond a personal prototype.
