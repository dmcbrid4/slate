...
- **A real gap, not just a documented free-tier limit: finished matches get stuck showing a stale "live" score forever.** `LiveTennisHttpClient.listMatches`'s type only accepts `status: 'live' | 'upcoming'` (`src/providers/live-tennis/http.ts:60`); once a real match ends, the provider stops returning it under either filter, our refresh cycle stops fetching it, and `readGraph` has no staleness/expiry logic — so the canonical row is left frozen at whatever status it last held, almost always `'live'`, indefinitely. Verified against the live database: a match observed as `'live'` during earlier #39 testing was still recorded `'live'` with no further updates until the next real request refreshed it. This is worse than the documented "no completed results" limitation — it doesn't just fail to show a final score, it can actively misrepresent an over match as still in progress. The client already exposes `getMatch(matchId)` (any status, by ID), so resolving this doesn't need new provider access — it needs a small new refresh resource that re-checks previously-seen non-final matches past their expected window. Scoped, low-cost, not yet implemented; worth doing before or independent of the paid-tier decision below.
