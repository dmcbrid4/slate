...
#38 is complete: `tests/refresh-repository.test.ts` synthetically covers `src/db/refresh-repository.ts`'s quota, lease, and completion/failure transitions with no PostgreSQL service required. The committed `main` branch still has no database driver, credentials, or environment configuration — that is intentional through #38.

#39 (opt-in real tennis scoreboard vertical slice) is a materially bigger step than the Phase 2 work so far: every task through #38 was disconnected from the running product by design, but #39 requires an actual PostgreSQL connection to exist and, in real mode, an actual call to Live Tennis API using `LIVE_TENNIS_API_KEY` and its live quota. Confirmed with the user: spending real Live Tennis API Free quota (80-call daily ceiling) against the product route is expected. Preserve mock startup as the default and never mix fictional and real tennis events in one list; free mode covers upcoming/live data only.

The PostgreSQL blocker is resolved. The user provisioned a dedicated Supabase project (`slate`) and supplied its transaction-pooler connection string via `DATABASE_URL` in `.env.local` (gitignored, alongside `LIVE_TENNIS_API_KEY`; never committed or printed). `src/db/client.ts` adds `getDb()` — a `postgres` (postgres.js) client wrapped in `drizzle-orm/postgres-js`, with `prepare: false` since Supabase's transaction pooler does not support server-side prepared statements. `npm run db:migrate` now applies `drizzle/*.sql` to a live instance (`drizzle.config.ts` loads `.env.local` itself via `process.loadEnvFile()`, since `drizzle-kit` only auto-loads a plain `.env`); all 14 tables were confirmed present via an independent `information_schema` query, not just the migrate command's own success message. `client.ts` intentionally has no `server-only` guard — `src/db` stays the neutral, directly-testable layer it already was, and the server-only boundary belongs at the composition root (`src/server/scoreboard.ts`) once this client is actually wired in. `tests/architecture.test.ts`'s allowlist was extended for `db`'s now-legitimate `postgres`/`drizzle-orm/postgres-js` imports — the first real driver instantiation in that layer.

`getInitialScoreboardData` in `src/server/scoreboard.ts` still calls only `mockScoreboardRepository`; nothing in the running app reads or writes the real database yet. That wiring — swapping in `createDrizzleScoreboardRepository(getDb())` behind whatever opt-in condition #39 specifies, preserving mock as the default — is what #39 itself should do.
