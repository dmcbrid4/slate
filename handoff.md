...
#39 (opt-in real tennis scoreboard vertical slice) is a materially bigger step than the Phase 2 work so far: every task through #38 was disconnected from the running product by design, but #39 requires an actual PostgreSQL connection to exist and, in real mode, an actual call to Live Tennis API using `LIVE_TENNIS_API_KEY` and its live quota. Before starting it, confirm with the user: which PostgreSQL instance to point at (local, and how it's provisioned), and that spending real Live Tennis API Free quota (80-call daily ceiling) against the product route is expected. Preserve mock startup as the default and never mix fictional and real tennis events in one list; free mode covers upcoming/live data only.
