...
The next task is **#40 — Free-slice validation and paid-capability decision**: measure real data quality and product value against the free tier's actual limits (no completed-results listing, no full rankings, no draws — confirmed firsthand during #39, not just from documentation) before considering a paid provider tier. This is explicitly a product/cost decision, not an implementation task — decide with the user before writing code. Note that the whole-graph read's occasional pooled-connection timeout (above) is a real signal worth weighing in that decision, separate from the paid-tier question: it will matter more once #42's narrower queries are in place regardless of provider tier.
